...
run_system() {
  ...
  d.stat "no password"
  $DOCKER_RUN "qemu-system-x86_64 -m "$VM_MEMORY" -smp $(nproc --all) -nic user,hostfwd=tcp::"$OTHER_PORT"-:8000 -drive file=windows10.qcow2 -usbdevice tablet -display vnc=127.0.0.1:1"                         
